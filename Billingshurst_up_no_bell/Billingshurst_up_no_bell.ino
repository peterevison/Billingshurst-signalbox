/*
  Billingshurst signal box up line
  PE 28/91/26

  Billingshurst_up_no_bell

*/

//global variables
const int SIGNAL_13    = 10;  // read down distant (signal 13) input D10
const int BLOCK_INSTR  = 12;  // read block instrument
...
void loop() {
  ...
  while  (digitalRead(BLOCK_INSTR))  delay(interval);  // pause until up block instrument is set to "Line Clear"
  delay(5e3);
  digitalWrite(lamp[1], LOW); // switch on lamp 1 (LOW gives lamp on)
  while  (digitalRead(SIGNAL_13))  delay(interval);  // pause at lamp 1 until up home lever 13 is reversed (all clear, on)
  delay(15e3);
